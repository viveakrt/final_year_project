...
export const generateFruits = (numFruits) => {
    for (var i = 0; i < numFruits; i++) {
        ...
        switch (newFruit.name) {
            case "apple":
                randomXPosition = generateRandomPosition(-120 * camera.aspect,
                    120 * camera.aspect
                );
                ...
                break;
            case "banana":
                randomXPosition = generateRandomPosition(-200 * camera.aspect,
                    200 * camera.aspect
                );
                ...
                break;
            case "bomb":
                randomXPosition = generateRandomPosition(-110 * camera.aspect,
                    110 * camera.aspect
                );
                randomYPosition = generateRandomPosition(-290, -190);
                newFruit.position.set(randomXPosition, randomYPosition, 100);
...
const animateHandTrail = (hand, camera, fruitsObjects) => {
    ...
    for (
        var vertexIndex = 0; vertexIndex < handGeometry.vertices.length; vertexIndex++
    ) {
